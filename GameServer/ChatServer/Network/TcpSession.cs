    ...
    public abstract class TcpSession
    {
        ...
        byte[] Serialize(IMessage message)
        {
            ...
            using (BinaryWriter writer = new BinaryWriter(stream))
            {
                ...
                long messageStartPosition = stream.Position; // 실제 데이터 시작위치
                message.Serialize(writer);
                long messageEndPosition = stream.Position; // 실제 데이터 끝위치
                long messageLength = messageEndPosition - messageStartPosition; // 실제데이터 길이
            ...
            while (SendQueue.TryDequeue(out ArraySegment<byte> segment))
            {
                try
                {
                    ...
                    while (offset < segment.Count)
                    {
                        ...
                        if (bytesSent <= 0)
                        {
                            Console.WriteLine($"[TCP Session] : Something went wrong.");
                            Disconnect();
                            return;
                        }
            ...
            while (IsConnected)
            {
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        if (message == null)
                        {
                            Console.WriteLine($"[{nameof(TcpSession)}] Invalid messageType {messageType}.");
                            break;
                        }
